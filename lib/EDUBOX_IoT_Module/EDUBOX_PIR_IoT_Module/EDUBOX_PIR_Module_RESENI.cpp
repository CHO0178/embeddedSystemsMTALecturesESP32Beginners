...
#include <WebServer.h>
#include "EDUBOX_PIR_Module_page_RESENI.hpp"
...
void example_handlerRoot_PIRModule() {
    String page = FPSTR(RES_EXAMPLE_PIR_MODULE_HTML);
...
void exercise_timeStamp_handlerRoot_PIRModule() {
    String page = FPSTR(RES_EXERCISE_TIMESTAMP_PIR_MODULE_HTML);
    server_PIR_Module.send(200, "text/html; charset=utf-8", page);
}
...
void exercise_extendedJSON_handlerRoot_PIRModule() {
    String page = FPSTR(RES_EXERCISE_EXTENDEDJSON_PIR_MODULE_HTML);
    server_PIR_Module.send(200, "text/html; charset=utf-8", page);
}
...
void exercise_extendedJSON_handlerData_PIRModule() {
    ...
    String json = "{";
    json += "\"detection\": " + String(detected ? "true" : "false") + ", ";
    json += "\"status\": \"" + String(detected ? "motion" : "no_motion") + "\", ";
...
void exercise_alarm_handlerRoot_PIRModule() {
    String page = FPSTR(RES_EXERCISE_ALARM_PIR_MODULE_HTML);
    server_PIR_Module.send(200, "text/html; charset=utf-8", page);
}
...
void exercise_alarm_handlerData_PIRModule() {
    ...
    if (armed && detected) {
        alarmLatched = true;
        digitalWrite(LEDPIN, HIGH);
        Serial.println("PIR: ALARM – detekován pohyb!");
...
void exercise_alarm_handlerAck_PIRModule() {
    alarmLatched = false;
    digitalWrite(LEDPIN, LOW);
    Serial.println("Alarm potvrzen a vynulován.");
    server_PIR_Module.send(200, "text/plain", "ACK");
}

void exercise_alarm_handlerArm_PIRModule() {
    armed = true;
    Serial.println("Systém aktivován.");
    server_PIR_Module.send(200, "text/plain", "ARMED");
}
...
void exercise_alarm_handlerDisarm_PIRModule() {
    ...
    alarmLatched = false;
    digitalWrite(LEDPIN, LOW);
    Serial.println("Systém deaktivován.");
    server_PIR_Module.send(200, "text/plain", "DISARMED");
}
...
void exercise_alarm_setup_PIRModule() {
    ...
    server_PIR_Module.on("/ack", exercise_alarm_handlerAck_PIRModule);
    server_PIR_Module.on("/arm", exercise_alarm_handlerArm_PIRModule);
    server_PIR_Module.on("/disarm", exercise_alarm_handlerDisarm_PIRModule);
